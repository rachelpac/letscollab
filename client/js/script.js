...
function showUserCollab() {
  ...
  if (localStorage.getItem("loggedinuserid") != "null") {
            ...
            if (data == false) {
              ...
              data.forEach((row) => {
                mycollablist.innerHTML +=
                  ...
                  row.Title +
                  "</td>" +
                  '<td><a class="btn-floating btn-large waves-effect waves-light" collab-id="' +
                  row.CollaborationID +
                  '"><i class="material-icons">send</i></a></td>' +
                  "</tr>";
              });
            }
          })
          .then(() => {
            mycollablistbtns = document.querySelectorAll(
              "#mycollablisttbl a.btn-floating.btn-large.waves-effect.waves-light"
            );
            for (var loop = 0; loop < mycollablistbtns.length; loop++) {
              mycollablistbtns[loop].addEventListener(
                "click",
                showMyCollab,
                false
              );
    ...
    mycollablisthead.hidden = true;
    mycollablist.innerHTML =
      "<tr>" + "<td> Please log in to view your collaborations </td>" + "</tr>";
    myrequests.innerHTML =
      "<tr>" + "<td> Please log in to view your collaborations </td>" + "</tr>";
  }
}
...
function showMyCollab() {
    ...
    .then((data) => {
      if (data == false) {
        ...
        fetch("../api/api.php?getData=displaylocrequests", {
            ...
            if (data == false) {
              ...
              data.forEach((row) => {
                mycollablocreq.innerHTML +=
                  ...
                  '<td><a class="btn-floating btn-large waves-effect waves-light green" loc-id="' +
                  row.LocationID +
                  '" collab-id="' +
                  row.CollaborationID +
                  '" loc-request-id="' +
                  row.LocationRequestID +
                  '" loc-search-id="' +
                  row.LocationSearchID +
                  '"><i class="material-icons">done</i></a></td>' +
                  '<td><a class="btn-floating btn-large waves-effect waves-light red" loc-request-id="' +
                  row.LocationRequestID +
          ...
          .then(() => {
            approvelocbtns = document.querySelectorAll(
              "#mycollablocreqtbl a.btn-floating.btn-large.waves-effect.waves-light.green"
            );
            for (var loop = 0; loop < approvelocbtns.length; loop++) {
              approvelocbtns[loop].addEventListener(
                "click",
                approveLocationRequests,
                false
              );
            }
          })
          .then(() => {
            denylocbtns = document.querySelectorAll(
              "#mycollablocreqtbl a.btn-floating.btn-large.waves-effect.waves-light.red"
            );
            for (var loop = 0; loop < denylocbtns.length; loop++) {
              denylocbtns[loop].addEventListener(
                "click",
                denyLocationRequests,
                false
              );
            }
          });
        ...
        mycollabloc.innerHTML =
          ...
          "</tr>";
        mycollablocreqtbl.innerHTML =
          "<tr>" + "<td> Location has been found </td>" + "</tr>";
      }
    });
  ...
  fetch("../api/api.php?getData=displayteamrequests", {
      ...
      if (data == false) {
        ...
        data.forEach((row) => {
          mycollabteamreq.innerHTML +=
            ...
            "</td>" +
            '<td><a class="btn-floating btn-large waves-effect waves-light"><i class="material-icons">person</i></a></td>' +
            '<td><a class="btn-floating btn-large waves-effect waves-light green" tm-role="' +
            row.Role +
            '" user-id="' +
            row.UserID +
            '" collab-id="' +
            row.CollaborationID +
            '" tm-request-id="' +
            row.TeamMemberRequestID +
            '" tm-search-id="' +
            row.TeamMemberSearchID +
            '"><i class="material-icons">done</i></a></td>' +
            '<td><a class="btn-floating btn-large waves-effect waves-light red" tm-request-id="' +
            row.TeamMemberRequestID +
            '"><i class="material-icons">clear</i></a></td>' +
            "</tr>";
        });
    ...
    .then(() => {
      approveteambtns = document.querySelectorAll(
        "#mycollabteamreqtbl a.btn-floating.btn-large.waves-effect.waves-light.green"
      );
      for (var loop = 0; loop < approveteambtns.length; loop++) {
        approveteambtns[loop].addEventListener(
          "click",
          approveTeamRequests,
          false
        );
    ...
    .then(() => {
      denyteambtns = document.querySelectorAll(
        "#mycollabteamreqtbl a.btn-floating.btn-large.waves-effect.waves-light.red"
      );
          ...
          if (data == false) {
            ...
            data.forEach((row) => {
              collabpost.innerHTML +=
                ...
                '<div class="row">' +
                '<div class="section col s6" loc-search-id="' +
                row.LocationSearchID +
                '">' +
                ...
                row.City +
                " - " +
                row.LocationBookingFee +
                "</p>" +
                "</div>" +
                '<div class="section col s6" tm-search-id="' +
                row.TeamMemberSearchID +
                '">' +
                "<h6> I NEED PEOPLE </h6>" +
                "<p>" +
                row.Role +
                " - " +
                row.TeamMemberBookingFee +
                "</p>" +
                "</div>" +
                "</div>" +
                '<button id="' +
                row.CollaborationID +
                '"class="btn waves-effect waves-light teal darken-3" type="button">JOIN<i class="material-icons right">send</i></button>' +
                "</div>" +
                "</div>" +
                "</div>";
            });
...
function showAddLocation() {
  ...
  if (
    (checkaddlocation.checked || checksearchlocation.checked) &&
    (checkaddmember.checked || checksearchmember.checked)
  ) {
...
function showErrorTitle() {
  if (ctitle.validity.valueMissing) {
    ...
    ctitleerror.setAttribute(
      "data-error",
      "Title can only be " + ctitle.maxLength + " characters"
    );
...
function showErrorLocationUsername() {
  if (locationuname.validity.valueMissing) {
    ...
    locationunameerror.setAttribute(
      "data-error",
      "Username must be at least " + locationuname.minLength + " characters"
    );
...
function showErrorLocationCity() {
  if (lcity.validity.valueMissing) {
    ...
    lcityerror.setAttribute(
      "data-error",
      "City must be at least " + lcity.minLength + " characters"
    );
  } else if (lcity.validity.tooLong) {
    lcityerror.setAttribute(
      "data-error",
      "City can only be " + lcity.maxLength + " characters"
    );
...
function showErrorLocationBookingFee() {
  if (lbookingfee.validity.valueMissing) {
    ...
    lbookingfeeerror.setAttribute(
      "data-error",
      "Booking Fee must be a min of  " + lbookingfee.min
    );
...
function showErrorTeamMemberBookingFee() {
  if (tmbookingfee.validity.valueMissing) {
    ...
    tmbookingfeeerror.setAttribute(
      "data-error",
      "Booking Fee must be a min of  " + tmbookingfee.min
    );
...
function showErrorUsername() {
  if (uname.validity.valueMissing) {
    ...
    unameerror.setAttribute(
      "data-error",
      "Username can only be " + uname.maxLength + " characters"
    );
  } else {
...
function showErrorPassword() {
  if (pword.validity.valueMissing) {
    ...
    pworderror.setAttribute(
      "data-error",
      "Password can only be " + pword.maxLength + " characters"
    );
...
function showErrorEmail() {
  if (email.validity.valueMissing) {
    ...
    emailerror.setAttribute(
      "data-error",
      "Email must be at least " + email.minLength + " characters"
    );
  } else if (email.validity.tooLong) {
    emailerror.setAttribute(
      "data-error",
      "Email can only be " + email.maxLength + " characters"
    );
  } else if (email.validity.patternMismatch) {
    emailerror.setAttribute(
      "data-error",
      "Entered value not a valid email address"
    );
  } else {
...
function showErrorInstagramHandle() {
  if (ighandle.validity.valueMissing) {
    ighandleerror.setAttribute(
      "data-error",
      "You must enter a Instagram Handle"
    );
...
function showErrorFirstName() {
  if (fname.validity.valueMissing) {
    ...
    fnameerror.setAttribute(
      "data-error",
      "First Name can only be " + fname.maxLength + " characters"
    );
...
function showErrorLastName() {
  if (lname.validity.valueMissing) {
    ...
    lnameerror.setAttribute(
      "data-error",
      "Last Name can only be " + lname.maxLength + " characters"
    );
...
function showErrorCity() {
  if (loccity.validity.valueMissing) {
    ...
    loccityerror.setAttribute(
      "data-error",
      "City can only be " + loccity.maxLength + " characters"
    );
...
function showErrorLocationDescription() {
  if (locdescript.validity.valueMissing) {
    locdescripterror.setAttribute(
      "data-error",
      "You must enter a Location Description"
    );
  } else {
    locdescripterror.setAttribute(
      "data-success",
